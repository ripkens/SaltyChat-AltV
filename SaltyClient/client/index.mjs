    ...
    OnEstablishCall = (player) => {
        ...
        if (this.VoiceClients.has(playerId)) {
            ...
            this.ExecuteCommand(new PluginCommand(Command.PhoneCommunicationUpdate, this.ServerUniqueIdentifier, new PhoneCommunication(voiceClient.TeamSpeakName, native.getZoneScumminess(native.getZoneAtCoords(ownPosition.x, ownPosition.y, ownPosition.z)) +
                native.getZoneScumminess(native.getZoneAtCoords(playerPosition.x, playerPosition.y, playerPosition.z)), 1.2, true, null)));
        }
    }
    ...
    PlayerStateUpdate = () => {
        ...
        try {
            ...
            this.VoiceClients.forEach((voiceClient, playerId) => {
                ...
                this.ExecuteCommand(new PluginCommand(Command.PlayerStateUpdate, this.ServerUniqueIdentifier, new PlayerState(voiceClient.TeamSpeakName, nPlayerPosition, voiceClient.VoiceRange, voiceClient.IsAlive, 1.0)));
            });
        } catch (error) {
